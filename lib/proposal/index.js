...
app.get('/proposal/:id/comments', function (req, res) {
  ...
  api.comment.getFor(req.params.id, function (err, comments) {
    if (err) return _handleError(err, req, res);
...
app.post('/proposal/:id/comment', restrict, function (req, res) {
  ...
  api.proposal.comment(req.params.id, comment, function (err, proposal) {
    ...
    // START: Send email notification to participants
    api.proposal.get(req.params.id, function(err, proposal) {

      var ids = [];

      for(var i in proposal.participants) {
        if(proposal.participants[i]._id !== undefined) {
          ids.push(proposal.participants[i]._id);
        }
      }

      Citizen.findByIds(ids, function(err, users) {
        notify('comment').to(users).withData({ proposal_id: req.params.id, comment: comment }).now();
      });

    });
    // END: Send email notification to participants
...
app.post('/proposal/:id/vote', restrict, function (req, res) {
  ...
  api.proposal.vote(proposal, citizen, value, function (err, proposal) {
    if (err) return _handleError(err, req, res);
