...
exports.all = function all(fn) {
  ...
  Law
  .find()
  .select('id lawId mediaTitle tag participants votes createdAt updatedAt closingAt status open closed')
  .populate('tag', 'id hash name color')
...
exports.get = function get(id, options, fn) {
  ...
  var query = Law.findById(id);
  
  if (options.populate) query.populate(options.populate);

  query.populate('tag');

  query.exec(function (err, law) {
    if (err) {
      log('Found error %s', err);
      return fn(err);
    };
